...
% ASS1 MATLAB code for ass1.fig
%      ASS1, by itself, creates a new ASS1 or raises the existing
%      singleton*.
%
%      H = ASS1 returns the handle to a new ASS1 or the handle to
%      the existing singleton*.
%
%      ASS1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ASS1.M with the given input arguments.
%
%      ASS1('Property','Value',...) creates a new ASS1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ass1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ass1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ass1

% Last Modified by GUIDE v2.5 16-Oct-2020 12:08:20
...
% Choose default command line output for ass1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ass1 wait for user response (see UIRESUME)
...
ans = multiply(a,b);
set(handles.product,"string",ans)
